...
<x-app-layout>
    ...
    <x-container-section>
        <div class="flex">
            ...
            <a href="{{ route('assign_permission', $role->id) }}" class="btn btn-sm btn-success space-x-2 text-white">
                ...
                @else
                    @foreach($permissions as $permission)
                        <tr>
                            ...
                            <td class="capitalize">
                                <form action="{{ route('remove_permission_from_role', [$role->id, $permission->id]) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
    ...
    <x-container-section>
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('assign_user_roles', $role->id) }}" class="btn btn-sm btn-success text-white">
                <i class="fa-solid fa-plus"></i>
                ...
                @else
                    @foreach($assignedUsers as $assignedUser)
                        <tr>
                            ...
                            <td class="truncate">
                                <form action="{{ route('remove_user_from_role', [$role->id, $assignedUser->id]) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="link text-red-600 hover:text-red-800 transition-all">
